...
import cn.huahai.tel.service.ex.PasswordNotMatchException;
import cn.huahai.tel.service.ex.VerifyNotPass;
...
//  生成验证码所需要的注入
	...
	public User login(HttpServletRequest request,String received,User user) {
		try {
			if(getJson(request.getSession()).get(user.getUname()).equals(user.getPassword())) {
